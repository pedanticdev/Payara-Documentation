...
`filter`:: The notifier only responds to events at or above this level. (INFO, WARNING, SEVERE)
...
* `PayaraNotifierConfiguration`: Holds the common attributes like _enabled_ and _filter_, and is required for storing it in the `domain.xml` configuration file.
