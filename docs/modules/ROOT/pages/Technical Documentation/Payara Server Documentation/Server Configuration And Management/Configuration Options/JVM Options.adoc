...
JVM options can be created via the Admin Console, the `create-jvm-option` asadmin command, or multiple can be created with the `create-jvm-options` asadmin command. The jvm option key value pair must be enclosed in quotation marks.

*Example 1:*:: `asadmin create-jvm-options "-Djava.awt.headless\=true"`

*Example 2:*:: `asadmin create-jvm-option -Xmx2g`
