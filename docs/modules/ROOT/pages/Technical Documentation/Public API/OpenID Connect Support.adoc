...
|===
| Option | MP Config Property a| Required/Type | Description | Default value a| Requirements
...
|
a| This is an array of strings in the form `paramName=value`.
...
TIP: When defining this property using MP Configuration, the value of the entire property must correspond to a String that follows the standard URL query pattern, i.e. `param1=value1&param2=value2&param2=value+with+spaces`. +
Key entries can be repeated.

See the section <<extra-parameters-evaluation>> for more details.
...
|
a|Corresponds to an EL expression that can be used to set extra configuration parameters to be sent to the OpenID Connect provider.
...
TIP: When configured, this property takes precedence over other extra parameter sources.
...
TIP: Values can use further expressions to be configurable at runtime.
